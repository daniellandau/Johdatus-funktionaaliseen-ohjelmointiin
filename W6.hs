...
pilko s
  | nSpaces == 0 = Nothing
  | nSpaces == 1 = Just $ let [a,b] = words s in (a,b)
  | otherwise = Nothing
  where nSpaces = length $ filter (`elem` " ") s

tarkastaNumero (etu, suku)
  | (length $ filter (`elem` ['0'..'9']) (etu++suku)) == 0 
    = Just (etu,suku)
  | otherwise = Nothing

tarkastaIsotAlkukirjaimet (etu,suku)
  | all (`elem` ['A' .. 'Z']) $ map head [etu,suku]
    = Just (etu, suku)
  | otherwise = Nothing
...
chainList [] = Just []
chainList ms = foldr f (Just []) ms
  where f m acc = 
          m   ?> \l  -> 
          acc ?> \ls -> 
          Just (l:ls)
-- Vaihtoehtoisesti:  where f = liftM2 (:)
...
sumPos xs = let listOfMs = map (\x -> if x >= 0 then Just x else Nothing) xs
                mList = chainList listOfMs
            in mList ?> \xs -> Just $ sum xs
...
selectSum xs is = let listOfMs = foldr (\i acc -> (safeIndex xs i):acc) [] is
                      mList = chainList listOfMs
                      maybeSum ms = ms >>= \xs -> Just $ sum xs
                  in maybeSum mList
...
count x = modify (increment x)
  where increment x [] = [(x,1)]
        increment x (p:pairs) =
          case p of 
            (y,count) -> 
              if (x == y) 
              then (x,count+1) : pairs      -- valmis
              else p : increment x pairs    -- jatketaan
